...
# ===================== IGA改进遗传算法=====================
# 适应度：综合满载率 = min(空间利用率, 载重利用率)
class IGA:
    def __init__(self, cargos, pop=150, iter=300):
        self.cargos = cargos
        self.pop = pop
        self.iter = iter
        self.best_fit = 0
    ...
    def fit(self, ind):
        fit, plan, _, _ = blf_decode(ind)
        return fit, plan

    def run(self):
        # 初始种群：按重泡比排序后对前半部分随机打乱
        ...
        print(f"车型2 - 开始进化，种群大小: {self.pop}，迭代次数: {self.iter}")

        pop_size = self.pop

        for gen in tqdm(range(self.iter), desc=f"车型2 进化中", unit="代"):
            # 计算种群中每个个体的适应度
            fit_plan_pairs = []
            for ind in pop:
        ...
        print(f"车型2 - 进化完成，最佳适应度: {self.best_fit:.2%}")
...
def calculate_min_trucks(cargos):
    ...
    with tqdm(total=max_trucks, desc="处理车辆", unit="辆") as pbar:
        while remaining_cargos:
            ...
            # 坐标转换：左下角原点 -> 右后下角原点
            print(f"\n  车辆 {truck_id} 详细装箱方案（坐标原点：右后下角）：")
            print(f"  {'货物ID':<10} {'类型':<10} {'姿态(l,w,h)':<20} {'放置坐标(x,y,z)':<30} {'尺寸(cm)':<15} {'重量(kg)':<10}")
            print(f"  {'-' * 95}")
            ...
            for i, g in enumerate(best_plan, 1):
                ...
                print(f"  {g['id']:<10} {type_name:<10} {str(pose):<20} {str(new_pos):<30} {f'{pose[0]}×{pose[1]}×{pose[2]}':<15} {g['weight']:<10}")
...
if __name__ == "__main__":
    ...
    print("\n===== 问题1–1 车型2 单车满载率最大化 =====")
    iga = IGA(cargos, pop=150, iter=300)
    best_fit, best_plan = iga.run()
    v = sum([g['vol'] for g in best_plan])
    w = sum([g['weight'] for g in best_plan])
    truck = Truck()
    ...
    for i, g in enumerate(best_plan, 1):
        ...
        print(f"{g['id']:<10} {type_name:<10} {str(pose):<20} {str(pos):<30} {f'{pose[0]}×{pose[1]}×{pose[2]}':<15} {g['weight']:<10}")
